...
// TheSportsDB league IDs (free tier). If lookuptable.php ever stops working on the
// free key for your account, standings will just keep the last successful value —
// see the README for a paid-key alternative if you need guaranteed reliability.
const LEAGUES = {
  epl:        { id: 4328, name: 'Premier League', short: 'EPL' },
  laliga:     { id: 4335, name: 'La Liga',         short: 'ESP' },
  seriea:     { id: 4332, name: 'Serie A',         short: 'ITA' },
  bundesliga: { id: 4331, name: 'Bundesliga',      short: 'GER' },
  ligue1:     { id: 4334, name: 'Ligue 1',         short: 'FRA' },
  mls:        { id: 4346, name: 'MLS',             short: 'USA' }
};
...
async function refreshFixturesAndStandings() {
  for (const [key, league] of Object.entries(LEAGUES)) {
    ...
    try {
      ...
      // Standings lookup can be restricted on some free API keys — log it, but don't crash the refresh
      console.error(`Standings refresh failed for ${league.name} (may need a paid API key):`, err.message);
    }
  }
