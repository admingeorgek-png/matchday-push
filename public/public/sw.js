...
async function fixtures(k){let c=L[k][2],n=new Date(),a=new Date(n),b=new Date(n);a.setDate(a.getDate()-1);b.setDate(b.getDate()+7);let x=await json(`https://api.football-data.org/v4/competitions/${c}/matches?dateFrom=${day(a)}&dateTo=${day(b)}`,{headers:{'X-Auth-Token':FD}});return(x.matches||[]).map(match).sort((a,b)=>new Date(a.date)-new Date(b.date))}
...
async function afGet(u){let res=await fetch(u,{headers:{'x-apisports-key':AF}});let j=await res.json();if(j.errors&&Object.keys(j.errors).length)throw Error(JSON.stringify(j.errors));return j}
const norm=s=>String(s||'').toLowerCase().replace(/[^a-z]/g,'');
app.get('/api/match-detail',async(q,r)=>{
 if(!AF)return r.status(400).json({available:false,error:'not_configured'});
 try{
  const{home,away,date}=q.query;
  if(!home||!away||!date)return r.status(400).json({available:false,error:'missing_params'});
  const d=String(date).slice(0,10);
  const fx=await afGet(`https://v3.football.api-sports.io/fixtures?date=${d}`);
  const list=fx.response||[];
  const h=norm(home),a=norm(away);
  const found=list.find(x=>{const th=norm(x.teams?.home?.name),ta=norm(x.teams?.away?.name);return(th.includes(h)||h.includes(th))&&(ta.includes(a)||a.includes(ta))});
  if(!found)return r.json({available:false,reason:'match_not_found'});
  const id=found.fixture.id;
  const[lu,st,ev]=await Promise.all([
   afGet(`https://v3.football.api-sports.io/fixtures/lineups?fixture=${id}`).catch(()=>({response:[]})),
   afGet(`https://v3.football.api-sports.io/fixtures/statistics?fixture=${id}`).catch(()=>({response:[]})),
   afGet(`https://v3.football.api-sports.io/fixtures/events?fixture=${id}`).catch(()=>({response:[]}))
  ]);
  r.json({available:true,status:found.fixture.status,score:found.goals,lineups:lu.response||[],statistics:st.response||[],events:ev.response||[]});
 }catch(e){r.status(500).json({available:false,error:e.message})}
});
